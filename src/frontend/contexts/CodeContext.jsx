...
export const CodeProvider = ({ children }) => {
  ...
  const generateCode = useCallback(
    async (shapes, connections, language = "javascript") => {
  ...
  const generateShapeCode = (shape, language, index) => {
    ...
    let code = `${indent}${comment} ${shape.type} - ${
      shape.text || "Step " + (index + 1)
    }\n`;
    ...
    switch (shape.type) {
      ...
      case "end":
        ...
        break;
      case "process":
        code += `${indent}${getProcessCode(
          shape.text || "process",
          language
        )}\n`;
        break;
      case "decision":
        code += `${indent}${getDecisionCode(
